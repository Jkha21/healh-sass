...
const STATUS_LABELS: Record<PatientStatus, string> = {
  active: "Active", pending: "Pending", priority: "Priority", complete: "Done",
};
...
const SCHED_DOT_CLASSES: Record<ScheduleItem["status"], string> = {
  now: "bg-green-500", upcoming: "bg-amber-400", done: "bg-gray-300",
};

const ALERT_DOT_CLASSES: Record<DashboardAlert["level"], string> = {
  urgent: "bg-red-600", warning: "bg-amber-400", info: "bg-blue-500",
};
...
function DashboardContent() {
  // useRouter only for page-level clicks (patients, appointments etc.)
  // Sidebar navigation is handled inside Sidebar.tsx itself
  const router      = useRouter();
  const { session } = useSession();
  const {
    patients, alerts, schedule, stats,
    activeTab, searchQuery,
    setActiveTab, setSearchQuery, fetchDashboardData,
  } = useDashboardStore();
  ...
  // Quick actions — inline router.push, no map needed
  const quickActions = [
  ...
  return (
    // No onNavigate prop — AppLayout/Sidebar handle sidebar routing internally
    <AppLayout
      title="Dashboard"
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-orange-500 via-orange-400 to-orange-200 p-7 mb-7 flex items-center justify-between gap-5">
        ...
        <div className="relative z-10">
          <h1 className="font-display text-2xl sm:text-[26px] font-extrabold text-white leading-tight mb-1.5">
            {getGreeting()}, {session?.name?.split(" ")[0] || "Doctor"}!
          </h1>
        ...
        <div className="rounded-2xl bg-white border border-orange-900/[.08] shadow-[0_8px_32px_rgba(232,85,32,.10)] overflow-hidden">
          ...
          <div className="flex gap-1 px-6 pt-3 border-b border-orange-100 overflow-x-auto">
            {(["all","active","pending","priority","complete"] as TabKey[]).map((tab) => (
              ...
              >
                {tab === "all" ? `All (${stats.appointments})` : tab.charAt(0).toUpperCase() + tab.slice(1)}
              </button>
            ))}
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[12.5px] text-orange-800/60 flex flex-wrap items-center gap-1.5">
                    <span>{p.age} yrs</span><span>·</span><span>{p.reason}</span>
                  </div>
                </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-2xl bg-white border border-orange-900/[.08] shadow-[0_8px_32px_rgba(232,85,32,.10)] overflow-hidden">
            <div className="px-6 py-5 border-b border-orange-100">
              ...
              <div className="text-[12.5px] text-orange-800/60 mt-0.5">{stats.priorityAlerts} require attention</div>
            </div>
            <div>
              {alerts.map((a: DashboardAlert, i: number) => (
                <div key={i} className="flex items-start gap-3 px-5 py-3.5 border-b border-orange-50 last:border-0 cursor-pointer hover:bg-orange-50/60 transition-colors">
        ...
        <div className="rounded-2xl bg-white border border-orange-900/[.08] shadow-[0_8px_32px_rgba(232,85,32,.10)] overflow-hidden">
          ...
          <div>
            {schedule.map((s: ScheduleItem, i: number) => (
              ...
              >
                <span className="font-display text-[13px] font-bold text-orange-500 min-w-[52px]">{s.time}</span>
                <div className="flex-1 min-w-0">
